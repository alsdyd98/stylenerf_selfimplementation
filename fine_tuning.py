# #l2 = [1.8, 1.85, 1.9, 1.95, 2.0]
# #pl = [0.075, 0.080, 0.085, 0.09]
...
#         if i == 1.9 and j == 0.085:
#             continue
#         l2_value = l2[i]
#         pl_value = pl[j]
#         directory = "./fine_tuning/l2_{0}_pl_{1}".format(i,j)
       
#         python inversion.py --network ffhq_1024.pkl --encoder network-snapshot-000170.pkl --l2_lambda 1.9 --pl_lambda 0.095 --encoder_z True --target ./face.jpg --seed 104 --num-steps 10000 --outdir directory
...
for i in range(len(l2)):
    for j in range(len(pl)):
        l2_value = float(l2[i])
        pl_value = float(pl[j])
        directory = "./fine_tuning/l2_{0}_pl_{1}_again".format(l2[i],pl[j])
